...
class AwsProvider(Provider):
    ...
    def iam_access_snapshot(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"AWS IAM access review failed: {exc}") from exc
    ...
    def log_monitoring_snapshot(self) -> dict[str, Any]:
        ...
        try:
            for trail in trails.describe_trails(includeShadowTrails=False).get("trailList", []):
            ...
            raise RuntimeError(f"AWS log monitoring failed: {exc}") from exc

        total = max(len(trails.describe_trails(includeShadowTrails=False).get("trailList", [])), 1)
    ...
    def config_and_auth_snapshot(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"AWS config/auth snapshot failed: {exc}") from exc
    ...
    def encryption_snapshot(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"AWS encryption snapshot failed: {exc}") from exc
    ...
    def retention_snapshot(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(f"AWS retention snapshot failed: {exc}") from exc
