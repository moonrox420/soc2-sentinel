    ...
    def _handle_get(self) -> None:
        ...
        if path == "/api/telemetry/events":
            try:
                ...
                events = TELEMETRY.get_recent_events(limit=limit, tenant_id=get_current_tenant_id())
    ...
    def _handle_post(self) -> None:
        ...
        if path == "/api/policy/evaluate":
            try:
                ...
                TELEMETRY.emit(
                    ...
                    details={"passed": report.passed, "violations": len(report.violations)},
                )
                self._send_json(report.to_dict())
                return
            except PermissionError as pe:
        ...
        if path == "/api/tokens/create":
            try:
                ...
                token = TokenManager.create_token(user_id_obj, expires_in_seconds=expires)
        ...
        if path == "/api/vault/seal":
            try:
                ...
                TELEMETRY.emit(
                    ...
                    outcome="SUCCESS",
                    details={"merkle_root": block.merkle_root, "block_hash": block.block_hash},
                )
        ...
        if path == "/api/vendor-risk/vendors":
            try:
                ...
                q = SecurityQuestionnaire(**q_data) if q_data else SecurityQuestionnaire()
                vendor = Vendor(
                    ...
                    data_classification=DataClassification(v_data.get("data_classification", DataClassification.INTERNAL.value)),
                    owner_email=v_data.get("owner_email", ""),
                    status=VendorStatus(v_data.get("status", VendorStatus.UNDER_REVIEW.value)),
                ...
                TELEMETRY.emit(
                    ...
                    outcome="SUCCESS",
                    details={"risk_score": saved.risk_score, "status": saved.status.value},
                )
        ...
        if path == "/api/access-review/decide":
            try:
                ...
                if not camp:
                    self._send_json({"error": f"Campaign '{camp_id}' not found"}, status=404)
                    return
                ...
                if not found_item:
                    self._send_json({"error": f"Item '{item_id}' not found in campaign"}, status=404)
                    return
        ...
        if path == "/api/access-review/signoff":
            try:
                ...
                self._send_json({"status": "completed", "sign_off_hash": sig_hash, "campaign": camp.to_dict()})
                return
            except ValueError as ve:
                self._send_json({"error": str(ve)}, status=400)
                return
        ...
        if path == "/api/audit-rooms":
            try:
                ...
                p_end = body.get("period_end", datetime.now(timezone.utc).strftime("%Y-%m-%d"))
