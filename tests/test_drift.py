...
def test_drift_single_run_baseline(tmp_path: Path):
    ...
    rep = detect_configuration_drift(
        tmp_path, current_date="2026-09-20", record_audit=False
    )
...
def test_drift_two_runs_comparative(tmp_path: Path):
    ...
    for col, cid, metrics in [
        ...
        (
            "config_drift",
            "CC6.2",
            {"open_security_groups": 0, "unapproved_changes_detected": 0},
        ),
        (
            "encryption_status",
            "C1.2",
            {"unencrypted_data_stores": 0, "encrypted_at_rest_percentage": 100.0},
        ),
    ...
    for col, cid, metrics in [
        ...
        (
            "config_drift",
            "CC6.2",
            {"open_security_groups": 2, "unapproved_changes_detected": 1},
        ),
        (
            "encryption_status",
            "C1.2",
            {"unencrypted_data_stores": 1, "encrypted_at_rest_percentage": 90.0},
        ),
...
def test_compare_collector_metrics_helper():
    items = _compare_collector_metrics(
        "unknown_collector", "X1.1", {}, {}, "2026-09-23T00:00:00Z"
    )
    assert items == []
